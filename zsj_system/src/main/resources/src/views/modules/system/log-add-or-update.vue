<template>
  <el-dialog
    ...
    <el-form-item label="操作名称" prop="operation">
      <el-input v-model="dataForm.operation" placeholder="操作名称"></el-input>
    </el-form-item>
    ...
    methods: {
      init (id) {
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.id) {
            this.$http({
              url: this.$http.adornUrl(`/system/log/info/${this.dataForm.id}`),
              method: 'get',
              params: this.$http.adornParams()
            }).then(({data}) => {
              if (data && data.code === 0) {
                this.dataForm.username = data.log.username
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/system/log/${!this.dataForm.id ? 'save' : 'update'}`),
              method: 'post',
