    ...
    methods: {
      init (id) {
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.uuid) {
            this.$http({
              url: this.$http.adornUrl(`/system/captcha/info/${this.dataForm.uuid}`),
              method: 'get',
              params: this.$http.adornParams()
            }).then(({data}) => {
              if (data && data.code === 0) {
                this.dataForm.code = data.captcha.code
                this.dataForm.expireTime = data.captcha.expireTime
              }
            })
          }
        })
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/system/captcha/${!this.dataForm.uuid ? 'save' : 'update'}`),
              method: 'post',
